...
- **Share links are file-level links.** Owners can create, list, and revoke short-lived file download links from the detail drawer, including labels, expiry windows, optional download caps, optional passwords, and recent access metadata. Recipients use a local download page. Project/folder share pages and richer audit views are still future work.
...
2. **Share-link audit controls** for richer activity views and exportable access history.
