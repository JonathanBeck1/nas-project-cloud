...
- **Project-first workspace** — Inbox, projects, categories, tags, smart views, archive, and a 6-digit-code device pairing flow.
...
- **No tag, category, or project management UI.** The schema and APIs are there; the editing screens aren't.
...
3. **Tag, category, and project management UI** so the schema's full surface is editable.
4. **Upload Center expansion**: failed/aborted history, auto-resume after refresh, retry, per-device filtering.
5. **CSRF + login rate limiting + session sliding expiration + last-seen tracking** as a hardening pass before any reverse-proxied deployment.
6. **Mobile sidebar + list-view mode + dark mode**.
7. **Video poster frames** (validate `ffmpeg` first) and a CAD preview strategy decision.
8. **Per-device source detection** so the `from-windows` and `from-mac` smart views actually fill.
