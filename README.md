...
- **Share links are file-level bearer links.** Owners can create, list, and revoke short-lived file download links from the detail drawer, including labels, expiry windows, optional download caps, and basic usage metadata. Project/folder share pages, password prompts, and richer audit views are still future work.
...
2. **Share-link audit controls** for password prompts, per-download access history, and richer activity views.
